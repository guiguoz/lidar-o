...
**⚠ Mise à jour 2026-08-07 — contrôle visuel OOM + diagnostic granularité :**

Comparaison pipeline (fd=2, σ=1.0) vs carte FFCO Grimbosq sur **emprise hull FFCO = 323,8 ha**
(et non les 600 ha de l'emprise pipeline — le clip est indispensable).

| classe | FFCO | pipeline |
|---|---|---|
| 406 | 19,3 % / 463 poly / **med 5,17 mm²** / 3,7 % < 1 mm² | 21,9 % / 702 / **2,02 mm²** / 11,1 % |
| 408 | 6,9 % / 432 / **2,31 mm²** / 13,4 % | 10,2 % / 394 / **2,23 mm²** / 9,1 % |
| 410 | 8,0 % / 363 / **1,39 mm²** / 32,0 % | 6,3 % / 264 / **1,38 mm²** / 31,4 % |

*(mm² = surface à l'impression 1:10 000)*

**Nouvelle métrique de suivi obligatoire :** médiane mm² par classe sur hull FFCO.
Quadruplet par classe : `couverture %`, `n polygones`, `médiane mm²`, `% < 1 mm²`.
Scripts : `scripts/sweep_holes.py`, `sweep_closing.py`, `sweep_hysteresis.py`, `sweep_sigma.py`.

**Conclusions du contrôle visuel :**
1. La **détection est bonne** — surface ×0,82, compte ×1,20. Le pipeline ne sur-détecte pas.
2. **408 et 410 sont bien calibrés** — médiane 2,23 vs 2,31 (408) et 1,38 vs 1,39 (410). σ=1.0 est
   le bon choix pour ces deux classes : σ=1.5 et σ=2.0 les dégradent.
3. **L'écart réel est la granularité 406** — 702 patches pipeline vs 463 FFCO pour la même surface.
   Médiane 2,02 mm² vs 5,17 mm² (×2,5 d'écart). La fragmentation est structurelle.

**Quatre leviers testés, tous réfutés :**

| Levier | Résultat | Valeur principale |
|---|---|---|
| `remove_holes` sweep [30→3000 m²] | Médiane 406 : 2,02→2,06 — inchangée | Scripts `sweep_holes.py` |
| Fermeture morphologique [3×3→7×7] | Médiane 2,02→2,16, max% 11→45 % à kernel=7 | `sweep_closing.py` |
| Hystérésis t_low [0.20→0.08] | Médiane 2,02→2,43 (pic), max% 11→92 % — percolation à t_low=0.16 | `sweep_hysteresis.py` |
| σ sweep [1.0→2.0] | Médiane 2,02→2,17 ; max% 11→35 % à σ=2.0 ; 408/410 dégradés | `sweep_sigma.py` |

**Conclusion — limite mesurée, pas un renoncement :**
La fragmentation du 406 à σ=1.0 est une propriété structurelle du champ LiDAR. Les patches sont
réellement séparés dans le champ de densité — aucune opération (raster ou vecteur) ne les fusionne
sans déclencher la percolation. La cible médiane 5,17 mm² n'est pas atteignable depuis ce signal.

Sur le bon côté de l'erreur : 700 patches là où le cartographe en dessinerait 463 est le **bon
côté de l'erreur pour un fond de travail** — le cartographe peut fusionner, pas inventer.

**`min_area` recalibrés sur hull FFCO** (suite contrôle visuel) :
- 406 : 100 m² (inchangé)
- 408 : 100 m² (50→100 ; ciblait 40,9→9,1 % < 1 mm², cible FFCO 13,4 %)
- 410 : 75 m² (20→75 ; sweep hull donne med=1,38 mm², pct=31,4 % — cibles FFCO 1,39/32,0 %)
...
| **Kilemäed** (EE) | Landes + marais, couvert ouvert | ×18 / ×73 / ∞ (406/408/410) | **Hors portée** |
| **Airelles** (FR) | Lande d'altitude + résineux épars | ×4,8 vs cible | **Hors portée** |

**Conclusion portabilité :** le pipeline est **portable techniquement** (8 étapes, 2 terrains hors
France, sans crash, H1–H8 documentés et corrigés). Il n'est **pas portable cartographiquement**
hors des forêts tempérées fermées.
...
**Cause racine de l'échec Kilemäed :** landes boréales et marais à sphaignes produisent un signal
HAG[0,3:3 m] identique à une forêt ralentissante. Le cartographe code ces formations en 401–403
(terrain découvert avec végétation basse) ; le pipeline les classe en 406/408/410. Les comptages
finals (sur emprise totale, sans hull clip) : 406 ×18, 408 ×73, 410 ∞ vs FFCO. Hors portée.
Voir `docs/portabilite.md` pour le détail H1–H8 et la banding documentation.
...
1. **Végétation basse en terrain ouvert praticable** — le mode principal (53,7 % du surplus
   Airelles, ~100 % Kilemäed). Pas de correctif connu par le signal seul.
...
3. **Zones humides / rocheuses** — marais codés en bleu par ISOM mais vus comme végétation basse
   par le LiDAR (Kilemäed : marais à sphaignes dominant). **Correctif identifié** : masque
...
| `remove_holes` = levier principal sur la granularité | Médiane 406 : 2,02→2,06 mm² sur sweep [30→3000 m²]. Les patches n'ont pas de trous significatifs. | Réfutée |
| Fermeture morphologique = agrégation sûre | Médiane 2,02→2,16, max%406 11→45 % dès kernel=5×5 — percolation immédiate | Réfutée |
| Hystérésis = fusion sans percolation | Médiane plafonne à 2,43 mm², max% saute à 36 % dès t_low=0,16 (−0,04 du seuil) | Réfutée |
| σ intermédiaire (1.5–2.0) = plateau médiane/concentration | Médiane 2,02→2,17, max% 11→35 % à σ=2.0 — transition brutale, pas de plateau | Réfutée |
